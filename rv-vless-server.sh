...
get_acme_cert() {
    ...
    echo ""
    _line
    echo -e "  ${W}开始 DNS 手动验证流程${NC}"
    echo -e "  ${Y}由于 80/443 端口被封，我们将通过添加 DNS TXT 记录来验证域名。${NC}"
    _line
    echo ""
    
    # 步骤 1: 获取 TXT 记录值
    _info "正在向 Let's Encrypt 请求验证信息..."
    ...
    # 执行第一次 issue 以获取记录
    local output
    output=$("$acme_sh" --issue --dns -d "$domain" --yes-I-know-dns-manual-mode-enough-go-ahead-please 2>&1)
    
    # 提取 Domain 和 TXT value
    local txt_domain=$(echo "$output" | grep "Domain: " | head -1 | awk -F"'" '{print $2}')
    local txt_value=$(echo "$output" | grep "TXT value: " | head -1 | awk -F"'" '{print $2}')
    
    if [[ -z "$txt_value" ]]; then
        # 假如之前已经申请过但没完成，可能需要 renew
        output=$("$acme_sh" --renew -d "$domain" --yes-I-know-dns-manual-mode-enough-go-ahead-please 2>&1)
        txt_domain=$(echo "$output" | grep "Domain: " | head -1 | awk -F"'" '{print $2}')
        txt_value=$(echo "$output" | grep "TXT value: " | head -1 | awk -F"'" '{print $2}')
    fi

    if [[ -z "$txt_value" ]]; then
        _err "无法获取 DNS 验证信息，请查看下方详细错误："
        echo "$output"
        return 1
    fi
    ...
    # 步骤 2: 显示信息并等待用户操作
    echo ""
    echo -e "  ${Y}请手动前往您的域名 DNS 服务商 (如阿里云/Cloudflare/腾讯云)，添加以下记录：${NC}"
    _dline
    echo -e "  记录类型 (Type):  ${G}TXT${NC}"
    echo -e "  主机记录 (Host):  ${G}${txt_domain%%.*}${NC}  (完整: $txt_domain)"
    echo -e "  记录值 (Value):   ${G}$txt_value${NC}"
    _dline
    echo ""
    echo -e "  ${C}添加完成后，请等待 1-2 分钟让解析生效。${NC}"
    read -rp "  解析生效后，请按回车键继续验证..." dummy
    
    # 步骤 3: 验证并安装
    _info "正在验证 DNS 记录并申请证书..."
    
    # 注意：手动模式验证使用的是 --renew 参数
    if "$acme_sh" --renew -d "$domain" --yes-I-know-dns-manual-mode-enough-go-ahead-please \
        --install-cert \
        --key-file       "$cert_dir/server.key"  \
        --fullchain-file "$cert_dir/server.crt" \
        --reloadcmd      "$reload_cmd" 2>&1 | grep -E "^\[|Verify finished|Cert success|error|Error" | sed 's/^/  /'; then
        
        if [[ -f "$cert_dir/server.crt" && -f "$cert_dir/server.key" ]]; then
            echo ""
            _ok "证书申请并安装成功！"
            _warn "注意：DNS 手动模式不支持自动续期，请在 90 天内重新运行脚本更新证书。"
        ...
        echo ""
        _err "验证失败！"
        _warn "可能原因：DNS 解析尚未生效，或 TXT 记录填写错误。"
        _warn "请检查后重新运行脚本。"
        return 1
    fi
}
